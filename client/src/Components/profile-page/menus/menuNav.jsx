...
const MenuNav = ({ menus }) => {
  ...
  const menuSelection = menuTypes.slice(0, menus.length - 1).map((menuType, idx) => {
    return (
      <Tab.Pane eventKey={menuType} key={idx} style={tabPaneStyles}>
  ...
  return (
    <Tab.Container defaultActiveKey={activeKey} fluid="true" style={{ width: '100%' }}>
      <Row style={navStyles} fluid="true">
        <MenuNavLinks>{menuLinks}</MenuNavLinks>
      </Row>
      <Container style={menuContentStyles} fluid="true">
        <Tab.Content>{menuSelection}</Tab.Content>
        <div style={{ width: '100%', textAlign: 'center' }}>
          <button style={viewMenuButtonStyle} size="lg">
            View full menu
          </button>
        </div>
      </Container>
